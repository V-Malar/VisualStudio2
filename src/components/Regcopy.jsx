...
export default function Register() {
    ...
    return (
        <div>
                                    ...
                                    <div key={index} className="no">
                                        {
                                            val.map((val1, indexa) => {
                                                return (
                                                    <>
                                             { text1.includes(val) && text2.includes(val)    ?   <label style={{backgroundColor: "darkred"}} className="topmove" key={index} onClick={()=>{console.log(val); setText1(val);setText2(val)}}>{val1}</label> : <label className="nohover" key={index} onClick={()=>{console.log(val);  setText1(val);setText2(val)}}>{val}</label>}
            ...
            <p><span className="thick" onClick={() => setCalendar("initial")}><i className="w3-jumbo w3-spin fa fa-trash" style={{ color: "green", cursor: "pointer" }}></i></span></p>
        </div >
    )
}